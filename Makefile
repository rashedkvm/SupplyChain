...
install: require-kapp ## Installs the supply chains in the group to your active context.
	kapp deploy $(NAMESPACE_FLAG) -a components.source.test.vmware.com -f components -f pipelines -f tasks -y --dangerous-allow-empty-list-of-resources
	kapp deploy $(NAMESPACE_FLAG) -a supplychain.source.test.vmware.com -f supplychains -y --dangerous-allow-empty-list-of-resources
...
uninstall: require-kapp ## Uninstalls the supply chains in this group from your active context.
	kapp delete $(NAMESPACE_FLAG) -a supplychain.source.test.vmware.com -y
	kapp delete $(NAMESPACE_FLAG) -a components.source.test.vmware.com -y
